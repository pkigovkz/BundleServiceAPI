...
data class NCALayerJSON(val version: String, val info: String, val updurl: String, val disturls: List<DistUrlJSON>, val bundles: List<BundleJSON>) {
	companion object {
		@JvmStatic fun parseJSON(json: String): NCALayerJSON {
			...
			return NCALayerJSON(jo.getString("version"), jo.getString("info"), jo.getString("updurl"), disturlList, bundleList)
		}
	}
